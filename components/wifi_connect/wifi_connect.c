...
esp_err_t w_init(void)
{
    ...
    w_netif = esp_netif_create_default_wifi_sta();
    if (w_netif == NULL) {
...
esp_err_t w_deinit(void)
{
    ...
    ESP_ERROR_CHECK(esp_wifi_clear_default_wifi_driver_and_handlers(w_netif));
    esp_netif_destroy(w_netif);
